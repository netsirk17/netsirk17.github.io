...
<p>Cabin would be an interesting parameter to look at but there were too many unique values of cabin (not passenger class as in first, second, third) to be a meaninful predictor in a small dataset. However cabins specify not just the class of a passenger but where on the ship the passenger is located. People closer to the lifeboats would have had a better chance of survival.
</p>  