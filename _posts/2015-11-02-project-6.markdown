...
<p>On the bright side, it turns out that gross revenue is not very highly correlated with rating at all! Of course I hadn't adjusted for inflation and as the oldest film in the dataset was from 1921 (The Kid) and the newest from 2016 (Zootopia, Deadpool), this may severly skew the data. 
</p>
...
<br>
<p>I extracted down key words from language-related features using sklearn's CountVectorizer. I wanted to see which actors, directors, plot and title key-words, genres and production companies most affect ratings. Are some Hollywood A-listers worth the huge cost of their contracts? Are some genres more inclined to higher ratings than others? 
</p>

<p>I limited the key words to the top 25 in each category as I did not want too many features to flow into a model with a very small number of samples.
</p>
...
<br>
<p>I trained and tested tree-based models to determine feature importance for ratings. Model performance was evaluated by examinging cross-val scores.
</p>

<p>Regressor and classifier models were both used. The float value of imdb Ratings (from 0-10) were used as the target variable for regressors and three classification bins (average, good, excellent) were used for classifiers.
...
<p> GridSearchCV DecisionTree Regressor performed the best overall, with a cross-validated R2 score of 1.0. 
</p>

<b>Regressor Tree Models</b>
<div>
  <a href="#">
    <img src="{{ site.baseurl }}/img/project-6/toptrees.png"></a>
</div>
